...
import (
	...
	"github.com/q-meet/m3u8Tool/parse"
	"github.com/q-meet/m3u8Tool/tool"
)
...
func (d *Downloader) download(segIndex int) error {
	...
	if ok && key != "" {
		fmt.Printf("key: %v %T, %v, %T \n ", key, d.result.M3u8.Keys[sf.KeyIndex].IV, key, d.result.M3u8.Keys[sf.KeyIndex].IV)
